...
def create_handlers() -> list:
    ...
    notifications_on_handler = CommandHandler(
        "notifications_on", notifications_on, filters.User(username=settings.ADMINS)
    )
    notifications_off_handler = CommandHandler(
        "notifications_off", notifications_off, filters.User(username=settings.ADMINS)
    )
...
def notifications_on(app: Application) -> None:
    ...
    notification_update_datetime = (
        datetime.combine(datetime.today(), notification_time)
...
async def notify(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if (
        not event
        or event.event_id in context.bot_data[settings.CLUB_CHANNEL]
        or event.canceled
    ):
        return
    ...
    context.bot_data[settings.CLUB_CHANNEL][event.event_id][
        "message_id"
...
async def notification_update(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    changed = [
        ...
        if event.__dict__[key]
        != context.bot_data[settings.CLUB_CHANNEL][event.event_id][key]
    ]
    ...
    await context.bot.edit_message_caption(
        chat_id=settings.CLUB_CHANNEL,
        message_id=context.bot_data[settings.CLUB_CHANNEL][event.event_id][
            "message_id"
        ],
        caption=event.get_notification_message(),
    )
...
def fetch_events() -> list:
    ...
    mr_events_params["started_before"] = (datetime.now() + timedelta(weeks=2)).strftime(
        "%Y-%m-%d"
    )
    response = requests.get(
        MR_EVENTS_URL, headers=MR_EVENTS_HEADERS, params=mr_events_params
    )
